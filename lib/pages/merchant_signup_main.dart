...
class _MerchantSignupMainState extends State<MerchantSignupMain> {
  ...
  bool _outletPictureUploading = false;
  bool _businessRegUploading = false;
  bool _logoUploading = false;
  bool _nicFrontUploading = false;
  bool _nicBackUploading = false;
  // Opening hours
        ...
        child: ListView(
          children: [
            ...
            _purpleUploadButton(
              ...
              isLoading: _outletPictureUploading,
              onPressed: () async {
                setState(() => _outletPictureUploading = true);
                final url = await uploadFileToFirebase(
                  fileType: "outlet_picture",
                  userId: _emailController.text.trim(),
                );
                setState(() => _outletPictureUploading = false);
                if (url != null) {
                  setState(() {
...
GestureDetector(
      ...
      color: _hasAgreed ? Colors.green : Colors.black,
          ...
          Center(
            child: _purpleUploadButton(
              ...
              isLoading: _businessRegUploading,
              onPressed: () async {
                setState(() => _businessRegUploading = true);
                final url = await uploadFileToFirebase(
                  fileType: "business_reg",
                  userId: _emailController.text.trim(),
                );
                setState(() => _businessRegUploading = false);
                if (url != null) {
                  setState(() {
  ...
  child: _purpleUploadButton(
    ...
    isLoading: _nicFrontUploading,
    onPressed: () async {
      setState(() => _nicFrontUploading = true);
      final url = await uploadFileToFirebase(
        fileType: "nic_front",
        userId: _emailController.text.trim(),
      ...
      setState(() => _nicFrontUploading = false);
      if (url != null) {
        setState(() {
  ...
  child: _purpleUploadButton(
    ...
    isLoading: _nicBackUploading,
    onPressed: () async {
      setState(() => _nicBackUploading = true);
      final url = await uploadFileToFirebase(
        fileType: "nic_back",
        userId: _emailController.text.trim(),
        validateNIC: true, // Enable NIC validation
      ...
      setState(() => _nicBackUploading = false);
      if (url != null) {
        setState(() {
