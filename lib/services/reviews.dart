...
Future<List<Review>> fetchReviewsByMerchant(
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print(
        'DEBUG: Failed to fetch reviews. Status: ${response.statusCode}, Body: ${response.body}',
      );
...
Future<bool> addReview({
  ...
  try {
    ...
    print(
      'DEBUG: Review data - Rating: $rating, Message: "${message.substring(0, message.length > 50 ? 50 : message.length)}${message.length > 50 ? '...' : ''}"',
    );
